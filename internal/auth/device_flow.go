...
func PollDeviceToken(appID, appSecret, baseURL, deviceCode string, interval, expiresIn int, onTick func(elapsed, total int)) (*TokenStore, error) {
	...
	for time.Now().Before(deadline) && attempts < maxPollAttempts {
		...
		case "expired_token", "invalid_grant":
			return nil, fmt.Errorf("授权码已过期，请重新执行 feishu-cli auth device")
		default:
			desc, _ := raw["error_description"].(string)
			if desc == "" {
				desc = errVal
	...
	if attempts >= maxPollAttempts {
		return nil, fmt.Errorf("超过最大轮询次数（%d 次），请重新执行 feishu-cli auth device", maxPollAttempts)
	}
	return nil, fmt.Errorf("授权超时（设备码已过期），请重新执行 feishu-cli auth device")
}
