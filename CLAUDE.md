...
**登录命令的四种模式**：
- `auth login --scope "..."`：显式请求一组 user scope，阻塞轮询到授权完成
- `auth login --domain <name> --recommend`：按业务域申请推荐 scope（推荐）
- `auth login --json`：阻塞轮询 + JSON 事件流输出到 stdout（AI Agent 推荐，配合 `run_in_background` 使用；非交互场景需搭配 `--scope` 或 `--domain/--recommend`）
- `auth login --no-wait --json`：只请求 device_code 并立即输出，同时缓存本次 requested scope，不启动轮询（两步模式第一步）
- `auth login --device-code <code> --json`：用已有 device_code 继续轮询（两步模式第二步）

**scope 策略**：CLI 在登录时会显式声明本次请求的 scope，不再依赖“后台全量兜底”。推荐先 `auth check --scope "REQ_SCOPES"` 预检，再通过 `auth login --scope "..."` 或 `auth login --domain ... --recommend` 按需补授权。`offline_access` 由 `device_flow.go` 强制注入；`auth:user.id:read` 等最小核心 scope 也会自动追加，用于识别当前登录用户。要真正拿到某个 scope，仍需先在飞书开放平台应用权限管理页面开通，然后重新 `auth login`。
...
```bash
# === 认证（Device Flow） ===
feishu-cli auth login --domain search --recommend  # 按业务域登录（推荐）
feishu-cli auth login --domain search --recommend --json   # JSON 事件流输出（AI Agent 推荐，配合 run_in_background）
feishu-cli auth login --domain vc --domain minutes --recommend --no-wait --json  # 两步模式第一步
...
feishu-cli bitable table update --base-token bscnxxxx --table-id tblxxx --name "新名"
feishu-cli bitable table delete --base-token bscnxxxx --table-id tblxxx

# Field（6）— update 改为 PUT 全量替换，search-options 路径是 /options
feishu-cli bitable field list --base-token bscnxxxx --table-id tblxxx
feishu-cli bitable field get --base-token bscnxxxx --table-id tblxxx --field-id fldxxx
feishu-cli bitable field create --base-token bscnxxxx --table-id tblxxx --config '{"name":"状态","type":"select"}'
feishu-cli bitable field update --base-token bscnxxxx --table-id tblxxx --field-id fldxxx --config '{"name":"新名","type":"text"}'
feishu-cli bitable field delete --base-token bscnxxxx --table-id tblxxx --field-id fldxxx
feishu-cli bitable field search-options --base-token bscnxxxx --table-id tblxxx --field-id fldxxx --query "关键词"

# Record（8）— upsert 根据 --record-id 自动 POST 创建或 PATCH 更新；history-list 改为 GET + query params
feishu-cli bitable record list --base-token bscnxxxx --table-id tblxxx --view-id vewxxx --limit 100
feishu-cli bitable record get --base-token bscnxxxx --table-id tblxxx --record-id recxxx
feishu-cli bitable record search --base-token bscnxxxx --table-id tblxxx --config-file search.json
feishu-cli bitable record upsert --base-token bscnxxxx --table-id tblxxx --config '{"fields":{"名称":"测试"}}'       # 创建
feishu-cli bitable record upsert --base-token bscnxxxx --table-id tblxxx --record-id recxxx --config '{"fields":{"状态":"完成"}}'  # 更新
feishu-cli bitable record batch-create --base-token bscnxxxx --table-id tblxxx --config-file records.json
feishu-cli bitable record batch-update --base-token bscnxxxx --table-id tblxxx --config-file records.json
feishu-cli bitable record delete --base-token bscnxxxx --table-id tblxxx --record-id recxxx
feishu-cli bitable record history-list --base-token bscnxxxx --table-id tblxxx --record-id recxxx --page-size 50

# View 基础（5）+ 视图配置（6 种 × get/set = 12）
feishu-cli bitable view list --base-token bscnxxxx --table-id tblxxx
feishu-cli bitable view get --base-token bscnxxxx --table-id tblxxx --view-id vewxxx
feishu-cli bitable view create --base-token bscnxxxx --table-id tblxxx --name "看板" --view-type kanban
feishu-cli bitable view rename --base-token bscnxxxx --table-id tblxxx --view-id vewxxx --name "新名"
feishu-cli bitable view delete --base-token bscnxxxx --table-id tblxxx --view-id vewxxx
# 视图配置：filter / sort / group / visible-fields / timebar / card 各自的 get + set（12 命令）
feishu-cli bitable view view-filter-get --base-token bscnxxxx --table-id tblxxx --view-id vewxxx
feishu-cli bitable view view-filter-set --base-token bscnxxxx --table-id tblxxx --view-id vewxxx --config '{"conjunction":"and","conditions":[...]}'
feishu-cli bitable view view-sort-set --base-token bscnxxxx --table-id tblxxx --view-id vewxxx --config '[{"field_id":"fld1","desc":false}]'   # 自动包装为 sort_info
feishu-cli bitable view view-group-set --base-token bscnxxxx --table-id tblxxx --view-id vewxxx --config '[{"field_id":"fld1"}]'              # 自动包装为 grouping
feishu-cli bitable view view-visible-fields-set --base-token bscnxxxx --table-id tblxxx --view-id vewxxx --config '{"visible_fields":["fld1","fld2"]}'
feishu-cli bitable view view-timebar-set --base-token bscnxxxx --table-id tblxxx --view-id vewxxx --config '{...}'
feishu-cli bitable view view-card-set --base-token bscnxxxx --table-id tblxxx --view-id vewxxx --config '{...}'

# Role（5）
feishu-cli bitable role list --base-token bscnxxxx
feishu-cli bitable role get --base-token bscnxxxx --role-id roxxx
feishu-cli bitable role create --base-token bscnxxxx --config-file role.json
feishu-cli bitable role update --base-token bscnxxxx --role-id roxxx --config '{...}'
feishu-cli bitable role delete --base-token bscnxxxx --role-id roxxx

# 高级权限（2）— 改为 PUT /advperm/enable?enable=true/false
feishu-cli bitable advperm enable --base-token bscnxxxx
feishu-cli bitable advperm disable --base-token bscnxxxx

# 数据聚合 + 工作流
feishu-cli bitable data-query --base-token bscnxxxx --config-file query.json     # POST /bases/{t}/data/query（base 级）
feishu-cli bitable workflow list --base-token bscnxxxx --page-size 50 --status enabled   # POST /workflows/list

# === 邮箱（Mail）— User Token 必需，首期不支持附件/CID ===
feishu-cli mail triage --folder INBOX --unread-only --page-size 20
feishu-cli mail triage --list-folders                                           # 列出可用文件夹
feishu-cli mail triage --query "周会"                                           # 专用 search 端点
...
feishu-cli mail send --to a@x.com,b@x.com --cc c@x.com --subject "会议" --body "<b>议程</b>" --html --confirm-send
...
# === 云盘增强（Drive）— 与 file/media 并存 ===
feishu-cli drive upload --file big.zip --folder-token fldxxx                    # 大文件自动分块
...
feishu-cli drive export-download --file-token boxxxx --output-dir ./exports     # 接力超时的 export
feishu-cli drive import --file report.docx --type docx --folder-token fldxxx    # 临时媒体上传，不留中间文件
feishu-cli drive move --file-token fldxxx --type folder --folder-token fldyyy   # folder 自动轮询 task_check
feishu-cli drive add-comment --doc https://xxx.feishu.cn/docx/yyy --content '[{"type":"text","text":"评论"}]'
feishu-cli drive add-comment --doc doccnxxxx --block-id blk_xxx --content '[{"type":"text","text":"局部评论"}]'
...
# === 视频会议与妙记（全部需 User Token，先 auth login）===
feishu-cli vc search --query "周会" --start 2026-03-20 --end 2026-03-28         # 多维过滤（至少一个过滤条件）
feishu-cli vc search --organizer-ids ou_xxx,ou_yyy                               # 按主持人过滤
feishu-cli vc notes --meeting-ids 69xxxx,70xxxx                                  # 批量查会议纪要
feishu-cli vc notes --minute-tokens obcnxxxx --with-artifacts                    # 妙记 token + AI 产物
feishu-cli vc notes --minute-tokens obcnxxxx --download-transcript --output-dir ./notes  # 下载逐字稿
feishu-cli vc notes --calendar-event-ids <event_id> --with-artifacts             # 日历事件直达
feishu-cli vc recording --meeting-ids 69xxxx                                     # 查会议录制，提取 minute_token
feishu-cli vc recording --calendar-event-ids <event_id>                          # 从日历事件反查录制
feishu-cli minutes get <minute_token> --with-artifacts                           # 获取妙记信息 + AI 产物
feishu-cli minutes download --minute-tokens obcnxxxx --output ./media            # 单条媒体下载
feishu-cli minutes download --minute-tokens t1,t2,t3 --output ./media --overwrite # 批量下载（最多 50）
feishu-cli minutes download --minute-tokens obcnxxxx --url-only                  # 只取下载 URL
...
| `/feishu-cli-vc` | 视频会议与妙记（多维搜索、三路径纪要、AI 产物、逐字稿、录制查询、媒体下载） | `/feishu-cli-vc` |
...
| 视频会议（User 身份） | `vc:meeting.search:read`、`vc:meeting.meetingevent:read`、`vc:note:read`、`vc:record:readonly`；calendar 路径需 `calendar:calendar:read`、`calendar:calendar.event:read` |
| 妙记（User 身份） | `minutes:minutes:readonly`、`minutes:minutes.artifacts:read`、`minutes:minutes.transcript:export`、`minutes:minutes.media:export` |
...
✅ bitable 全面切到 base/v3 API（/open-apis/base/v3/bases/{base_token}/...）
✅ bitable table list/get/create/update/delete（5）
✅ bitable field list/get/create/update/delete/search-options（6，update 为 PUT）
✅ bitable record list/get/search/upsert/batch-create/batch-update/delete/history-list（8，history-list 为 GET）
✅ bitable view list/get/create/delete/rename（5）
✅ bitable view view-filter/sort/group/visible-fields/timebar/card get/set（12，set 为 PUT）
✅ bitable role list/get/create/update/delete（5）
✅ bitable advperm enable/disable（PUT .../advperm/enable?enable=bool）
✅ bitable data-query（POST /bases/{t}/data/query，base 级）
✅ bitable workflow list（POST /workflows/list）
✅ mail send/draft-create/draft-edit/reply/reply-all/forward/message/messages/thread/triage（10 命令，默认存草稿，--confirm-send 才发送，triage --query 走 POST /search）
✅ drive upload（分块）/ download（流式）/ export（markdown 快捷 + sub-id + resume）/ export-download / import（medias/upload + ccm_import_open）/ move（folder 轮询）/ add-comment（富文本 + wiki 解析）/ task-result（通用异步查询）
...
✅ vc search（多维过滤：query/时间/organizer/participant/room）
✅ vc notes（三路径：meeting-ids / minute-tokens / calendar-event-ids，批量 + with-artifacts + download-transcript）
✅ vc recording（会议录制查询 → 提取 minute_token，批量）
✅ minutes get（妙记信息 + --with-artifacts 合并 AI 产物）
✅ minutes download（妙记媒体批量下载，SSRF/重定向/文件名解析/url-only）
```
